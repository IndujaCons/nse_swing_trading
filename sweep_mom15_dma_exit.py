"""
Mom15 200 DMA Force-Exit Sweep
Tests whether forcing out (and blocking entry of) stocks below their N-day
simple moving average improves Mom15 performance.

Configs tested:
  - Baseline  : no DMA check
  - 150 DMA   : force-sell + block-buy if price < 150d SMA
  - 200 DMA   : force-sell + block-buy if price < 200d SMA
  - 250 DMA   : force-sell + block-buy if price < 250d SMA
...
# DMA windows to test; None = baseline (no DMA check)
SWEEP_DMAS = [None, 150, 200, 250]
...
def compute_metrics(trades, label, avg_force_exits):
    if not trades:
        print(f"\n{label}: No trades")
        return None
    ...
    for _, row in df.iterrows():
        net, _ = compute_net_profit(
            row["pnl"], row["entry_price"], row["exit_price"], row["shares"]
        )
        nets.append(net)
    df["net_profit"] = nets
    ...
    print(f"\n{'='*62}")
    print(f"  {label}  |  avg force-exits/rebal: {avg_force_exits:.1f}")
    print(f"{'='*62}")
    ...
    return {
        ...
        "avg_force_exits": round(avg_force_exits, 1),
    }

# ── Pre-compute SMAs ──────────────────────────────────────────────────────────

def precompute_smas(stock_data, date_to_iloc, windows):
    """
    Pre-compute simple moving averages for each stock on each trading date.

    Returns dict: {window: {ticker: {date: sma_value}}}
    """
    sma_cache = {w: {} for w in windows}
    for ticker, df in stock_data.items():
        closes = df["Close"].values.astype(float)
        idx_map = date_to_iloc[ticker]
        for w in windows:
            ticker_sma = {}
            for d, ci in idx_map.items():
                start = max(0, ci - w + 1)
                window_data = closes[start:ci + 1]
                if len(window_data) >= w:
                    ticker_sma[d] = float(np.mean(window_data))
                else:
                    ticker_sma[d] = None   # not enough history
            sma_cache[w][ticker] = ticker_sma
    return sma_cache
...
def run_mom15_loop(stock_data, nifty50_data,
                   ...
                   pit_data, dma_window, sma_cache):
    """
    Run Mom15 rebalancing loop with optional DMA force-exit / entry-block.

    dma_window : None = baseline; int = SMA period (e.g. 200)
    sma_cache  : pre-computed SMA dict from precompute_smas()
    """
    ...
    trades       = []
    cash         = float(CAPITAL)
    force_exit_counts = []   # per rebalance count of DMA-triggered exits

    sma_ticker_map = sma_cache.get(dma_window, {}) if dma_window is not None else {}

    def get_sma(ticker, day):
        """Return SMA value or None if unavailable."""
        return sma_ticker_map.get(ticker, {}).get(day)
    ...
    for rebal_day in rebal_dates:
        ...
        # Keep existing if rank <= buffer_out (normal buffer rule)
        ...
        # ── DMA force-exit: remove from new_portfolio_tickers if below DMA ────
        # This applies regardless of rank (even if rank <= buffer_out).
        force_exits_this_rebal = 0
        if dma_window is not None:
            to_force_exit = set()
            for t in list(new_portfolio_tickers):
                if t not in current_tickers:
                    # New buy candidate — don't apply force-exit here (handled in buy block)
                    continue
                price = scores.get(t, {}).get("price")
                sma_val = get_sma(t, rebal_day)
                if price is not None and sma_val is not None and price < sma_val:
                    to_force_exit.add(t)
                    force_exits_this_rebal += 1
            new_portfolio_tickers -= to_force_exit

        force_exit_counts.append(force_exits_this_rebal)

        # ── Execute rebalance ──────────────────────────────────────────────────
        # Sell stocks leaving portfolio (normal exits + DMA force-exits)
        ...
        for t in to_sell:
            ...
            rank = ticker_rank.get(t, 999)
            sma_val = get_sma(t, rebal_day) if dma_window is not None else None
            if (dma_window is not None and sma_val is not None
                    and exit_price is not None and exit_price < sma_val
                    and rank <= BUFFER_OUT):
                exit_reason = f"DMA{dma_window}_FORCE_EXIT"
            else:
        ...
        # Buy new stocks — skip if price < DMA (entry block)
        ...
        for t in to_buy:
            ...
            if dma_window is not None:
                sma_val = get_sma(t, rebal_day)
                if sma_val is not None and price < sma_val:
    ...
    avg_force_exits = float(np.mean(force_exit_counts)) if force_exit_counts else 0.0
    return trades, avg_force_exits

# ── Main ──────────────────────────────────────────────────────────────────────

def main():
    end_date      = datetime.now()
    daily_start   = end_date - timedelta(days=PERIOD_DAYS + 700)  # extra buffer for SMA history
    bt_start_date = (end_date - timedelta(days=PERIOD_DAYS)).date()

    print("=" * 62)
    print("Mom15 DMA Force-Exit Sweep")
    print(f"DMA windows tested: {SWEEP_DMAS}")
    print("=" * 62)
    ...
    # ── Phase 6: Pre-compute SMAs (once, for all windows) ────────────────────
    dma_windows = [w for w in SWEEP_DMAS if w is not None]
    print(f"\nPre-computing SMAs for windows {dma_windows} ...")
    sma_cache = precompute_smas(stock_data, date_to_iloc, dma_windows)
    print("SMA pre-computation done.")

    # ── Phase 7: Sweep ────────────────────────────────────────────────────────
    results = []

    for dma_window in SWEEP_DMAS:
        label = "Baseline (no DMA)" if dma_window is None else f"{dma_window} DMA"
        print(f"\n--- Running {label} ---")

        trades, avg_force_exits = run_mom15_loop(
        ...
        print(f"  Completed: {len(trades)} trades, avg force-exits/rebal: {avg_force_exits:.2f}")
        r = compute_metrics(trades, label=label, avg_force_exits=avg_force_exits)
        if r:
            results.append(r)

    # ── Summary ───────────────────────────────────────────────────────────────
    print("\n\n" + "=" * 78)
    print("DMA Force-Exit Sweep — Mom15 (11yr PIT, 20L, 2-monthly rebalance)")
    print("=" * 78)
    print(f"{'Config':<20} {'Trades':>7} {'WR%':>6} {'PF':>6} {'Net%/yr':>9} {'NegYrs':>7} {'AvgForcExit':>12}")
    print("-" * 78)
    for r in results:
        print(f"{r['label']:<20} {r['trades']:>7} {r['wr']:>6.1f} "
              f"{r['pf']:>6.2f} {r['net_per_yr']:>+9.1f} {r['neg_years']:>7} {r['avg_force_exits']:>12.1f}")

    # Year-by-year table
    print("\n\nYEAR-BY-YEAR NET RETURNS (%)")
    all_years = sorted(set(yr for r in results for yr in r["yearly"]))
    header    = f"{'Year':>6}" + "".join(f"{r['label'][:16]:>18}" for r in results)
    print(header)
